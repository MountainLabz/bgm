        ...
        for layer in 0..CS {
            ...
            for forward in 0..CS {
                ...
                while bits_here != 0 {
                    ...
                    let quad = match face {
                        0 => get_quad(mesh_front, mesh_up, mesh_left, mesh_length, mesh_width, v_type, face),
                        1 => get_quad(mesh_front + mesh_length as usize, mesh_up, mesh_left, mesh_length, mesh_width, v_type, face),
                        2 => get_quad(mesh_up, mesh_front + mesh_length as usize, mesh_left, mesh_length, mesh_width, v_type, face),
                        3 => get_quad(mesh_up, mesh_front, mesh_left, mesh_length, mesh_width, v_type, face),
                        _ => unreachable!()
                    };
...
#[inline]
fn get_quad(x: usize, y: usize, z: usize, w: usize, h: usize, v_type: usize, normal: usize) -> u64 {
    ((v_type << 33) | (normal << 30) | (h << 24) | (w << 18) | (z << 12) | (y << 6) | x) as u64
    //face: (value >> 30) & 0b111,
    //v_type: value >> 33,
}
...
mod tests {
    ...
    fn doesnt_crash() {
        ...
        voxels[bgm::pad_linearize(0, 0, 1)] = 1;
        voxels[bgm::pad_linearize(0, 0, 2)] = 1;
        


        let mut mesh_data = crate::bgm::MeshData::new();
